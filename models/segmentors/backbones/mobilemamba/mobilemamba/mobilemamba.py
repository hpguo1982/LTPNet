...
from models.segmentors.backbones.mobilemamba import MODEL
...
class MobileMamba(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if x.is_cpu:
            ...
            device = x.device
        x1_0 = self.patch_embed[:2](x)
        x1_1 = self.patch_embed[2:4](x1_0)
        x1_2 = self.patch_embed[4:6](x1_1)
        x = self.patch_embed[6:](x1_2)
        # x = self.patch_embed(x)
        x1 = self.blocks1(x)
        x2 = self.blocks2(x1)
        x3 = self.blocks3(x2)
        ret.append(x1_1.to(device))
        ret.append(x1_2.to(device))
        ret.append(x1.to(device))
        ret.append(x2.to(device))
        ret.append(x3.to(device))
...
def MobileMamba_S6(num_classes=1000, pretrained=False, distillation=False, fuse=False, pretrained_cfg=None, model_cfg=CFG_MobileMamba_S6):
    ...
    if fuse:
        replace_batchnorm(model)
    pretrained_dict = torch.load("/home/chensen/msvm-unet-main/mobilemamba_s6.pth")
...
@MODEL.register_module
def MobileMamba_B4(num_classes=1000, pretrained=True, distillation=False, fuse=False, pretrained_cfg=None, model_cfg=CFG_MobileMamba_B4,ckpt: str = None):
    model = MobileMamba(num_classes=num_classes, distillation=distillation, **model_cfg)
    if fuse:
        replace_batchnorm(model)
    #"/home/chensen/mambayolo/mobilemamba_b4s.pth"
...
class Mobilemamba(nn.Module):
    def __init__(self, ckpt: str = None):
        ...
        parser.add_argument('-m', '--modelname', default="MobileMamba_B4")
        cfg = parser.parse_args()
        model_name = cfg.modelname
        self.net = model_dict[model_name](ckpt=ckpt)
    def forward(self,x):
        ret = self.net(x)
        return ret
# model = Mobilemamba().to("cuda")
